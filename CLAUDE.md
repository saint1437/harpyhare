...
1. **`config/presets.json` — один файл, два потребителя.** Его вшивает Rust (`include_str!` в `remote_presets.rs`, фолбэк, когда сеть недоступна) и импортирует фронт (`lib/presets.ts`). Правка формата ломает обе стороны сразу; публикация в блоб (`npm run presets:publish`) перекрывает вшитую копию в рантайме. Рядом лежит `config/answer-styles.json` (стиль ответа «много/меньше текста»), и он устроен НАОБОРОТ — потребитель у него один, фронт: системный промпт склеивается в `App.chatSystemPrompt`, Rust про этот файл не знает и в блоб он не публикуется. Не «унифицируй» их: у presets два потребителя и рантайм-обновление через блоб, у answer-styles ни того, ни другого.

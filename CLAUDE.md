...
- `stt.rs` — `GroqStt` (whisper-large-v3-turbo; язык из `Settings.stt_language`, "" = автоопределение; `stt_translate` → эндпоинт `/audio/translations` + модель whisper-large-v3 — turbo переводов не умеет, поле language не отправляется), error mapping; `transcribe_stream` — multipart с телом-стримом (chunked, длина неизвестна; Groq это принимает — проверено живым смоуком). Клиент пересоздаётся в `set_settings` при смене ключа/языка/перевода.

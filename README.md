...
Приложение само проверяет новые версии (на старте и раз в 6 часов) по `latest.json` из публичного репозитория [itech-releases](https://github.com/screenfriskofficial/itech-releases). При находке в шапке появляется бейдж с номером версии: клик → заметки релиза → «Обновить и перезапустить» (артефакт проверяется minisign-подписью). Ручная проверка — в настройках.
...
Скрипт бампит версию в трёх файлах, собирает подписанный бандл, публикует `itech_X.Y.Z_aarch64.app.tar.gz` + `latest.json` + `.dmg` в itech-releases и делает локальный коммит с тегом `vX.Y.Z` (пуш — вручную). Ключ ищется в `~/.tauri/itech.key` (переопределить: `ITECH_SIGNING_KEY`).

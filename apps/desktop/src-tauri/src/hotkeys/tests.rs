...
fn effective_falls_back_to_default_and_prefers_last_binding() {
    ...
    assert_eq!(effective(&[], ACTION_RECORD_MICROPHONE), primary_combo!("Q").current());
...
fn normalize_takes_a_combo_away_from_an_untouched_default() {
    let combo = action(ACTION_TELEPROMPTER).unwrap().default_combo.current();
    let mut bindings = vec![binding(ACTION_SEND, combo)];
    normalize(&mut bindings);
    assert_eq!(effective(&bindings, ACTION_SEND), combo);
    assert_eq!(
        effective(&bindings, ACTION_TELEPROMPTER),
        "",
        "дефолтный владелец теряет сочетание"
    );
}

#[test]
fn normalize_leaves_untouched_defaults_out_of_the_list() {
    let mut bindings = vec![binding(ACTION_RECORD, primary_combo!("R").current())];
    normalize(&mut bindings);
...
fn migration_moves_legacy_fields_and_skips_untouched_defaults() {
    let mut raw = serde_json::json!({
        "hotkey": "Cmd+Shift+X",
        "toggle_hotkey": action(ACTION_TOGGLE_WINDOW).unwrap().default_combo.current(),

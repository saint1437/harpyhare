...
import { listeningPresentation, type ListeningState, type WordEmphasis } from "@/lib/listening";
import { cn } from "@/lib/utils";
import { CaptureMeter } from "./CaptureMeter";

const WORD_CLASS: Record<WordEmphasis, string> = {
  normal: "text-fg",
...
export function ListeningStatus({
  ...
  const dict = useDict();
  const copy = dict.hud.listeningStatus;
  const { tone, animated, emphasis } = listeningPresentation(value);
  return (
    ...
    <span
      role="group"
      aria-label={dict.hud.statusBar.listeningLabel}
      className="flex min-w-0 shrink-0 items-center gap-1.5 rounded-md bg-surface px-1.5 py-0.5 ring-1 ring-inset ring-line"
    >
      <CaptureMeter tone={tone} animated={animated} />
      <span className={cn("text-caption whitespace-nowrap", WORD_CLASS[emphasis])}>
        {dict.common.listening[value].word}
      </span>
      <IconButton
        title={paused ? copy.resumeTitle : copy.pauseTitle}

...
function answerButton(): HTMLButtonElement {
  const button = screen.getByText("Ответить").closest("button");
...
describe("AutoTranscript", () => {
  it("показывает реплики с метками говорящих", () => {
    ...
    expect(screen.getByText("Интервьюер:")).not.toBeNull();
    expect(screen.getByText("Я:")).not.toBeNull();
  ...
  it("нечего отправлять — кнопка занята, и это сказано словами", () => {
    ...
    expect(screen.getByText("Всё услышанное уже ушло в чат.")).not.toBeNull();
  });
  ...
  it("в мгновенном режиме кнопки нет — отправка происходит сама", () => {
    ...
    expect(screen.getByText("Отвечаю на реплики собеседника сама.")).not.toBeNull();
  });

  it("пустая расшифровка честно говорит, что слушает", () => {
    renderPanel({ turns: [], pendingCount: 0 });
    expect(screen.getByText("Слушаю — реплики появятся здесь.")).not.toBeNull();
  });
